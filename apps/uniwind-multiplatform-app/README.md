...
- The [Uniwind](https://github.com/uni-stack/uniwind) package which lets you use Tailwind CSS in react-native.
  - (This repo includes a [patch](../../patches/uniwind+1.3.2.patch) to enable the `focus` class modifier to be used on `Pressable` components.)
...
The [themed components](./components) demonstrate how to use Uniwind class names and the custom colors to provide consistent text, button, and link appearance. These are used both in the [home screen](<./app/(tabs)/index.tsx>) and the [tab layout for web](./components/app-tabs.web.tsx).
...
The [css-wrapped-components.tsx](./components/css-wrapped-components.tsx) file provides CSS-enabled wrappers for components from Expo and other packages, enabling them to use Uniwind class names. These wrappers use the `withUniwind()` API from Uniwind.
